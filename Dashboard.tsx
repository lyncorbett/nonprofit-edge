...
import { 
  ...
  Target, Calendar, Heart, BookOpen
} from 'lucide-react';
    ...
    <div className="min-h-screen bg-slate-50 flex">
        ...
        <div className="mb-8">
          <img 
            ...
            onError={(e) => {
              ...
              e.currentTarget.parentElement!.innerHTML = '<div style="font-weight:800;color:#0D2C54;font-size:18px;">THE NONPROFIT EDGE</div>';
            }}
          />
        </div>
      ...
      <main className="flex-1 ml-[280px] p-8">
        <div className="max-w-[1000px]">
          ...
          {/* Today's Insight - with commitment CTA */}
          {(() => {
            const insights = [
            ...
            const todaysInsight = insights[dayOfYear % insights.length];
            
            return (
              <div className="bg-gradient-to-r from-[#0D2C54] to-[#1a3a5c] rounded-2xl px-8 py-6 shadow-lg">
                <div className="flex items-center justify-between mb-4">
                  <span className="text-[11px] text-white/50 uppercase tracking-wider font-semibold">Today's Insight</span>
                  <Lightbulb className="w-5 h-5 text-[#D4A84B]" />
                </div>
                <div className="flex items-start gap-4 mb-5">
                  <div className="text-4xl text-[#0097A9] font-serif leading-none">"</div>
                  <div className="flex-1">
                    <p className="text-lg italic text-white/90 leading-relaxed mb-2">
                      {todaysInsight.text}
                    </p>
                    <span className="text-sm font-medium text-[#0097A9]">— {todaysInsight.author}</span>
                  </div>
                </div>
                <div className="border-t border-white/10 pt-4">
                  <button
                    onClick={() => setShowCommitmentModal(true)}
                    className="w-full flex items-center justify-center gap-3 bg-[#D4A84B] hover:bg-[#c49a3d] text-white rounded-xl py-3 px-4 font-semibold transition-colors"
                  >
                    <Target className="w-5 h-5" />
                    Make a Leadership Commitment Today
