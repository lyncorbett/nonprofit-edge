...
interface DashboardProps {
  ...
  onLogout: () => void
  supabase?: any // Optional supabase client for role checking
}

// Admin access state interface
  ...
  const checkAdminAccess = async () => {
    if (!supabase) {
      ...
      const isAdmin = isOwner || 
        user.email === 'asha@thepivotalgroup.com' || 
        user.email === 'info@thepivotalgroup.com'
  ...
  return (
    <div className="flex min-h-screen bg-gray-50 font-sans">
      ...
      <aside className="w-52 bg-white border-r border-gray-200 flex flex-col fixed h-screen overflow-y-auto">
        ...
        {/* Owner: sees all 4 links                     */}
        {/* Admin: sees Content Manager + Platform Admin */}
        {/* ============================================ */}
        {!adminAccess.loading && adminAccess.isAdmin && (
          <div className="py-3 border-t border-gray-100">
            <div className="px-4 mb-2 text-xs font-semibold uppercase tracking-wider text-gray-400">Admin</div>
            ...
            <a onClick={() => onNavigate('admin')} className="flex items-center gap-3 px-4 py-2.5 cursor-pointer text-gray-600 hover:bg-gray-50 transition-all">
              ...
              <span className="flex-1 text-sm font-medium text-orange-600">Platform Admin</span>
            </a>
            ...
            {adminAccess.isOwner && (
              <a onClick={() => onNavigate('team-access')} className="flex items-center gap-3 px-4 py-2.5 cursor-pointer text-gray-600 hover:bg-gray-50 transition-all">
                <span className="w-8 h-8 bg-violet-100 rounded-lg flex items-center justify-center text-sm">🔐</span>
                <span className="flex-1 text-sm font-medium text-violet-600">Manage Admin Access</span>
              </a>
            )}
