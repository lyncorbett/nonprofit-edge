...
import { 
  FolderOpen, Download, Star, Settings, MessageCircle, Lightbulb,
  ChevronDown, ChevronRight, ArrowRight, X, Send, Check, Loader2,
  Calendar, Clock, Target
} from 'lucide-react';

// Logo - uses SVG from public folder
...
const DashboardV2: React.FC = () => {
  ...
  const commitmentOptions: CommitmentOption[] = [
    ...
    { icon: Target, text: "Discuss at next board meeting" },
    ...
    try {
      ...
      const response = await fetch('/api/ask-professor', {
        ...
        body: JSON.stringify({
          ...
          userName: 'Lyn',
          organizationName: 'The Pivotal Group',
          focusArea: 'Board Engagement'
        })
      });
    ...
    <div style={{ 
      ...
      <aside style={{
        ...
        <nav style={{ marginBottom: '24px' }}>
          ...
          <NavLink href="/my-downloads" icon={Download}>My Downloads</NavLink>
          <NavLink href="/favorites" icon={Star}>Saved Favorites</NavLink>
        </nav>

        {/* Recent Activity - NO View All */}
        ...
        <div style={{ maxWidth: '1000px' }}>
          ...
          <div style={{
            ...
            <div style={{ textAlign: 'right', minWidth: '120px' }}>
              <div style={{ fontWeight: 600, color: '#0D2C54', fontSize: '15px' }}>Unknown</div>
              <div style={{ fontSize: '11px', color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '1px' }}>Quote of the Day</div>
            </div>
          </div>
        </div>
      </main>
...
const ActivityItem: React.FC<{ name: string; time: string; completed?: boolean }> = ({ name, time, completed }) => (
  <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', padding: '8px 0' }}>
    ...
    <div>
      <div style={{ fontSize: '14px', fontWeight: 500, color: '#334155' }}>{name}</div>
