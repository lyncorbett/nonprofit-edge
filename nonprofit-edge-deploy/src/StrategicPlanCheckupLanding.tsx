...
/**
 * Strategic Plan Check-Up Landing Page
 * The Nonprofit Edge
 */
    ...
    const runAnimation = () => {
      ...
      setTimeout(() => {
        ...
        const interval = setInterval(() => {
          p += 2;
          setProgress(p);
          if (p >= 15 && !analyzeSteps.includes(0)) setAnalyzeSteps(prev => [...prev, 0]);
          if (p >= 40 && !analyzeSteps.includes(1)) setAnalyzeSteps(prev => [...prev, 1]);
          if (p >= 65 && !analyzeSteps.includes(2)) setAnalyzeSteps(prev => [...prev, 2]);
          if (p >= 90 && !analyzeSteps.includes(3)) setAnalyzeSteps(prev => [...prev, 3]);
          if (p >= 100) {
      ...
      <section style={{ padding: '80px 0 100px' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 24px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '60px', alignItems: 'center' }}>
          ...
          <div style={{ borderRadius: '16px', overflow: 'hidden', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25)' }}>
            <img 
              src="/images/strategic-plan-hero.png" 
              alt="Papers flying - your strategic plan shouldn't be chaos"
      ...
      <section id="features" style={{ padding: '80px 0 40px' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 24px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '24px', maxWidth: '1100px', margin: '0 auto' }}>
            {[
              { title: 'Goal Analysis', desc: 'Every goal scored for clarity, specificity, and measurability. See exactly which ones need work.', img: '/images/card-goal-analysis.jpg' },
              { title: 'SMART Rewrites', desc: 'Vague objectives transformed into specific, measurable goals you can actually track and report on.', img: '/images/card-smart-rewrites.jpg', position: 'top' },
              { title: 'Implementation Check', desc: "Does your plan have owners? Timelines? Tracking mechanisms? We'll tell you what's missing.", img: '/images/card-implementation.jpg' },
            ].map((card, i) => (
              <div key={i} style={{ position: 'relative', height: '320px', borderRadius: '16px', overflow: 'hidden', cursor: 'pointer' }}>
                <div style={{
                  position: 'absolute', top: 0, left: 0, width: '100%', height: '100%',
                  backgroundImage: `url('${card.img}')`, backgroundSize: 'cover',
                  backgroundPosition: card.position === 'top' ? 'center top' : 'center'
                }} />
                <div style={{
                  position: 'absolute', top: 0, left: 0, width: '100%', height: '100%',
